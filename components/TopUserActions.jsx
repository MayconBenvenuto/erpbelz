...
export default function TopUserActions({ currentUser, onRefresh, onLogout, isRefreshing = false }) {
  ...
  async function handleChangePassword(e){
    ...
    if (pwdForm.nova !== pwdForm.confirm) { toast.error('Confirmação não bate'); return }
    setSubmitting(true)
    try {
      ...
      if (!res.ok) { toast.error(data.error || 'Falha na troca'); return }
      toast.success('Senha alterada com sucesso')
      setPwdForm({ atual:'', nova:'', confirm:'' })
      setShowPwdDialog(false)
      // Refresh dados usuário
    ...
    <div className="hidden md:flex items-stretch gap-3" ref={containerRef}>
      <div className="flex items-center gap-3 pl-3 pr-4 py-2 rounded-xl bg-card border border-border shadow-sm relative">
        ...
        <div className="min-w-0 leading-tight">
          ...
          <p className="text-sm font-semibold truncate" title={currentUser.email}>{currentUser.nome}</p>
        </div>
        ...
        <div className="flex items-center gap-1">
          ...
          <Button variant="ghost" size="icon" aria-label="Mais ações" title="Mais ações" onClick={()=>setMenuOpen(o=>!o)} className="h-8 w-8 rounded-lg text-muted-foreground hover:text-primary hover:bg-primary/10">
        ...
        {menuOpen && (
          <div className="absolute top-full right-2 mt-2 w-48 rounded-md border bg-popover shadow-lg z-50 p-1 animate-in fade-in-0 zoom-in-95">
            <button
              onClick={() => { setMenuOpen(false); setShowPwdDialog(true) }}
      ...
      <Dialog open={showPwdDialog} onOpenChange={(o)=>{ if (!currentUser.must_change_password) setShowPwdDialog(o) }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Alterar Senha</DialogTitle>
            <DialogDescription>
              {currentUser.must_change_password ? 'Defina sua senha pessoal para continuar.' : 'Atualize sua senha com segurança.'}
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleChangePassword} className="space-y-4">
            {!currentUser.must_change_password && (
              <div className="space-y-1">
                ...
                <Input id="pwd-atual" type="password" autoComplete="current-password" value={pwdForm.atual} onChange={e=>setPwdForm(p=>({...p,atual:e.target.value}))} required />
              </div>
            )}
            {currentUser.must_change_password && (
              <div className="space-y-1">
                <Label htmlFor="pwd-atual">Senha Inicial (opcional)</Label>
                <Input id="pwd-atual" type="password" autoComplete="current-password" value={pwdForm.atual} onChange={e=>setPwdForm(p=>({...p,atual:e.target.value}))} />
              </div>
            )}
            <div className="space-y-1">
              <Label htmlFor="pwd-nova">Nova Senha</Label>
              <Input id="pwd-nova" type="password" autoComplete="new-password" value={pwdForm.nova} onChange={e=>setPwdForm(p=>({...p,nova:e.target.value}))} required />
              <p className="text-[10px] text-muted-foreground">Mín 8, maiúscula, minúscula e número.</p>
            </div>
            <div className="space-y-1">
              <Label htmlFor="pwd-confirm">Confirmar Nova Senha</Label>
              <Input id="pwd-confirm" type="password" autoComplete="new-password" value={pwdForm.confirm} onChange={e=>setPwdForm(p=>({...p,confirm:e.target.value}))} required />
            </div>
            <div className="flex justify-end gap-2 pt-2">
              {!currentUser.must_change_password && (
                <Button type="button" variant="outline" onClick={()=>setShowPwdDialog(false)}>Cancelar</Button>
              )}
