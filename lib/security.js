...
// Verificação rígida apenas no lado do servidor (evita quebrar bundle cliente que só precisa de helpers leves)
if (typeof window === 'undefined') {
  const isTestEnv = process.env.NODE_ENV === 'test' || process.env.VITEST
  const isBuildPhase = process.env.NEXT_PHASE === 'phase-production-build'
  const isCI = !!process.env.CI
  if (!SECURITY_CONFIG.jwtSecret || SECURITY_CONFIG.jwtSecret.length < 32) {
    if (isTestEnv || isBuildPhase || isCI) {
      const gen = () => Array.from({ length: 6 }, () => Math.random().toString(36).slice(2, 8)).join('')
      SECURITY_CONFIG.jwtSecret = ('EPHEMERAL_' + gen() + gen()).slice(0, 48)
      // eslint-disable-next-line no-console
      console.warn('[SECURITY][EPHEMERAL] JWT_SECRET ausente/fraco. Usando segredo efêmero para testes/build. DEFINA JWT_SECRET forte em runtime!')
    } else {
      // eslint-disable-next-line no-console
      console.error('[SECURITY] JWT_SECRET ausente ou fraco (mínimo 32 chars). Abortando inicialização no servidor.')
      throw new Error('JWT_SECRET inválido')
    }
  }
} else {
  // No cliente não expomos o segredo.
}

// Permite forçar revalidação em runtime (ex: em handler crítico)
export function assertStrongSecret() {
  if (!SECURITY_CONFIG.jwtSecret || SECURITY_CONFIG.jwtSecret.length < 32 || SECURITY_CONFIG.jwtSecret.startsWith('EPHEMERAL_')) {
    throw new Error('JWT_SECRET inválido/efêmero em runtime')
  }
}
...
// verifyToken implementado abaixo

// Rate Limiting
// Verifica JWT emitido por generateToken
