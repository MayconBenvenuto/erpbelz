...
if (typeof window === 'undefined') {
  ...
  if (!SECURITY_CONFIG.jwtSecret || SECURITY_CONFIG.jwtSecret.length < 32) {
    if (isTestEnv) {
      // Gera um segredo efêmero apenas para contexto de teste (não usar em produção)
      const gen = () => Array.from({ length: 6 }, () => Math.random().toString(36).slice(2, 8)).join('')
      SECURITY_CONFIG.jwtSecret = gen().slice(0, 48) // ~48 chars
      // eslint-disable-next-line no-console
      console.warn('[SECURITY][TEST] JWT_SECRET ausente/fraco. Gerado segredo efêmero apenas para testes.')
    } else {
      // eslint-disable-next-line no-console
  ...
  // No cliente não expomos o segredo; funções de geração/validação de token não devem ser usadas aqui.
  // Mantemos silêncio para não poluir console local – se realmente precisar, use NEXT_PUBLIC_* separado.
}
