...
export function renderBrandedEmail({ title, contentHtml, ctaText, ctaUrl, preheader } = {}) {
	// Normaliza a base para evitar barra dupla e localhost em producao
	const rawBase = process.env.ERP_APP_URL || process.env.CRM_APP_URL || process.env.NEXT_PUBLIC_BASE_URL || 'https://admbelz.vercel.app'
	...
	<html lang="pt-br">
		<head>
			...
			<title>${safe(title || 'Sistema de Gestão - Belz')}</title>
		</head>
						...
						<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:640px;">
							...
							<tr>
								<td align="center" style="padding:16px 10px 40px;color:#6b7280;font-family: Arial, sans-serif;font-size:12px;">
									© ${new Date().getFullYear()} Belz Seguros — Esta é uma mensagem automática do Sistema de Gestão - Belz.
								</td>
							</tr>
						</table>
					</td>
