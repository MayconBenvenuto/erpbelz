...
export default function MovimentacaoSection({ currentUser, token: parentToken }) {
  ...
  useEffect(() => {
    ...
    return () => { if (typeof window !== 'undefined') window.removeEventListener('solicitacao:created', handler) }
  }, [loadSolicitacoes])

  const groupedByStatus = useMemo(() => {
    const g = {}; for (const s of SOLICITACAO_STATUS) g[s] = []
  ...
  const updateStatus = async (id, status, bypass = false) => {
    ...
    try {
      const res = await fetch(`/api/solicitacoes/${id}`, { method:'PATCH', headers:{ 'Content-Type':'application/json', ...(token?{Authorization:`Bearer ${token}`}:{}) }, body: JSON.stringify({ status }) })
      const data = await res.json().catch(()=>({}))
      if(!res.ok) {
    ...
    } catch { toast.error('Erro de conexão') } finally { setUpdating(p=>({...p,[id]:false})) }
  }
  ...
  const openDetails = async (id) => {
    setDetailLoading(true); setDetail(null); setDetailOpen(true)
    try {
      const res = await fetch(`/api/solicitacoes/${id}`, { headers: { ...(token?{Authorization:`Bearer ${token}`}:{}) } })
      const data = await res.json().catch(()=>({}))
      if(!res.ok) { toast.error(data.message || 'Erro ao carregar detalhes'); setDetailOpen(false) } else setDetail(data.data)
    } catch { toast.error('Erro de conexão'); setDetailOpen(false) } finally { setDetailLoading(false) }
  }
  ...
  const isAnalista = ['analista_implantacao', 'analista_movimentacao'].includes(currentUser?.tipo_usuario)
  ...
  return (
    <div className="grid gap-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h2 className="text-xl font-semibold tracking-tight">{isConsultor ? 'Minhas Movimentações' : 'Movimentação'}</h2>
          <p className="text-sm text-muted-foreground flex items-center gap-2">{isConsultor ? 'Acompanhe suas solicitações.' : 'Gerencie solicitações e acompanhe SLA.'} {reloading && <Loader2 className="h-4 w-4 animate-spin text-primary" />}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Total</CardTitle><CardDescription className="text-xs">Solicitações criadas</CardDescription></CardHeader><CardContent><div className="text-2xl font-bold">{stats.total}</div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Concluídas</CardTitle><CardDescription className="text-xs">Finalizadas com sucesso</CardDescription></CardHeader><CardContent><div className="text-2xl font-bold">{stats.concluidas}</div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Atrasadas</CardTitle><CardDescription className="text-xs">Ultrapassaram prazo</CardDescription></CardHeader><CardContent><div className="text-2xl font-bold text-red-600">{stats.atrasadas}</div></CardContent></Card>
      ...
      <div className="overflow-x-auto pb-2 -m-1 pl-1">
        <div className="flex gap-4 w-max pr-4">
          {SOLICITACAO_STATUS.map(status => {
            const statusColors = SOLICITACAO_STATUS_COLORS[status] || { bg: '#f6f6f6', text: '#333', border: '#e2e2e2' }
            return (
              <div key={status} className="border rounded-md bg-card flex flex-col max-h-[520px] shadow-sm overflow-hidden min-w-[260px] w-[260px]">
                <div className="p-2 border-b flex items-center gap-2 text-sm font-medium capitalize sticky top-0 z-10" style={{ backgroundColor: statusColors.bg, color: statusColors.text, borderColor: statusColors.border }}>
                        ...
                        {isAnalista && sol.atendido_por && String(sol.atendido_por) === String(currentUser?.id) && (
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <button type="button" disabled={updating[sol.id]} className="px-2 py-0.5 text-[11px] rounded bg-primary text-white hover:bg-primary/90 disabled:opacity-50">Status</button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="start" className="w-40">
                              ...
                              {SOLICITACAO_STATUS.filter(s => s !== sol.status).map(next => (
                                <DropdownMenuItem key={next} className="text-[12px] capitalize" disabled={updating[sol.id]} onClick={() => updateStatus(sol.id, next)}>
                                  {next}
                                </DropdownMenuItem>
                              ))}
                            </DropdownMenuContent>
      ...
      {isGestor && (
        <SimpleTimeline solicitacoes={solicitacoes} />
      )}
              ...
              <div className="space-y-4 text-sm">
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="col-span-2 text-xs mt-1">
                    <span className="font-medium">Consultor solicitante:</span> {detail.criado_por_nome ? (<>{detail.criado_por_nome} {detail.criado_por_email && <span className="text-muted-foreground">(&lt;{detail.criado_por_email}&gt;)</span>}</>) : '—'}
                  </div>
                </div>
                <div>
                  ...
                  <div className="grid gap-2 max-h-56 overflow-y-auto">
                    {Array.isArray(detail.arquivos) && detail.arquivos.length > 0 ? detail.arquivos.map(a => (
                      <div key={a.path || a.url} className="flex items-center gap-2 text-xs border rounded p-2 bg-background">
                        ...
                        {a.url && <a className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-primary text-white hover:bg-primary/90" href={a.url} target="_blank" rel="noopener noreferrer" download><FileDown className="h-3.5 w-3.5" /> Baixar</a>}
                      </div>
                  ...
                  <ol className="relative border-l ml-2 pl-4 space-y-3">
                    {(Array.isArray(detail.historico)?detail.historico:[]).slice(-30).map((ev,i)=>(
                      <li key={i} className="space-y-0.5">
                        <div className="absolute -left-[9px] bg-background border rounded-full h-4 w-4 flex items-center justify-center">{statusIcon(ev.status)}</div>
        ...
        {confirmStatus && (
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="text-xs">Alterar de <span className="font-medium">{confirmStatus.from}</span> para <span className="font-medium">{confirmStatus.to}</span>? Essa ação será registrada no histórico.</AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel disabled={confirmBusy} onClick={()=>setConfirmStatus(null)}>Cancelar</AlertDialogCancel>
              <AlertDialogAction disabled={confirmBusy} onClick={async ()=>{ if(!confirmStatus) return; setConfirmBusy(true); await updateStatus(confirmStatus.id, confirmStatus.to, true); setConfirmBusy(false); setConfirmStatus(null) }}>{confirmBusy ? 'Salvando...' : 'Confirmar'}</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        )}
      </AlertDialog>
