...
export default function UsersSection({
  ...
  const openMetaDialog = (user, targetValue) => {
    ...
    setMetaForm({
      valor_meta_input: new Intl.NumberFormat('pt-BR', {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }).format(Number(targetValue || 0)),
    })
  }
  const closeMetaDialog = () => {
    setMetaDialogUser(null)
    setMetaForm({ valor_meta_input: '' })
  }
  ...
  const handleSubmit = async (e) => {
    ...
    await onCreateUser({
      ...userForm,
      afterSuccess: () => {
        setUserForm({ nome: '', email: '', senha: '', tipo_usuario: 'analista_implantacao' })
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (!Array.isArray(userGoals)) return []

    let list = users.slice()
    if (search.trim()) {
      const q = normalize(search)
      list = list.filter((u) =>
        [u.nome, u.email, u.tipo_usuario].some((f) => normalize(f).includes(q))
      )
    }
    ...
    const enriched = list.map((u) => {
      ...
      const achieved = Number(
        goal?.valor_alcancado ?? implanted.reduce((s, p) => s + Number(p.valor || 0), 0)
      )
      const progress = target > 0 ? achieved / target : 0
      return {
        ...u,
        _progress: progress,
        _implantadas: implanted.length,
        _metaTarget: target,
        _metaAchieved: achieved,
      }
    })
    enriched.sort((a, b) => {
      switch (sortBy) {
        case 'progresso':
          return sortAsc ? a._progress - b._progress : b._progress - a._progress
        case 'propostas':
          return sortAsc ? a._implantadas - b._implantadas : b._implantadas - a._implantadas
        case 'nome':
        default:
  ...
  useEffect(() => {
    try {
      const saved = JSON.parse(
        localStorage.getItem('erp:users:filters') ||
          localStorage.getItem('crm:users:filters') ||
          '{}'
      )
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        'erp:users:filters',
        JSON.stringify({ search, roleFilter, sortBy, sortAsc })
      )
      try {
        localStorage.removeItem('crm:users:filters')
      } catch {}
    } catch {}
        ...
        <Dialog open={isUserDialogOpen} onOpenChange={setIsUserDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Adicione um novo usuário (gestor, gerente, analista de implantação, analista de
                movimentação, analista cliente ou consultor).
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setUserForm((prev) => ({ ...prev, tipo_usuario: value }))
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="analista_cliente">Analista Cliente</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            ...
            <div className="flex flex-wrap gap-2 items-center">
              ...
              <div className="flex items-center gap-1 text-xs border rounded-md px-2 py-1 bg-muted/40">
                ...
                <button
                  ...
                  className={`px-1 rounded hover:bg-muted ${sortBy === 'nome' ? 'font-semibold underline' : ''}`}
                >
                  Nome
                </button>
                <button
                  onClick={() => toggleSort('progresso')}
                  className={`px-1 rounded hover:bg-muted ${sortBy === 'progresso' ? 'font-semibold underline' : ''}`}
                >
                  Progresso
                </button>
                <button
                  onClick={() => toggleSort('propostas')}
                  className={`px-1 rounded hover:bg-muted ${sortBy === 'propostas' ? 'font-semibold underline' : ''}`}
                >
                  Implantadas
                </button>
                <button
                        ...
                        <TableRow key={user.id} className="hover:bg-muted/40">
                          <TableCell className="font-medium">
                            <div className="flex items-center gap-2">
                              <span className="truncate" title={user.nome}>
                                {user.nome}
                              </span>
                            </div>
                            <div className="md:hidden text-[11px] text-muted-foreground">
                              {user.email}
                            </div>
                          </TableCell>
                          ...
                          <TableCell>
                            <Badge
                              variant={
                                ['gestor', 'gerente'].includes(user.tipo_usuario)
                                  ? 'default'
                                  : 'secondary'
                              }
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                              ...
                              <div className="text-[10px] text-muted-foreground leading-tight">
                                ...
                                <div>
                                  Falta {formatCurrency(Math.max(0, targetValue - achievedValue))}
                                </div>
                              </div>
                            </div>
                          </TableCell>
                          {currentUser?.tipo_usuario === 'gestor' && (
                            <TableCell className="space-x-1">
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => openMetaDialog(user, targetValue)}
                              >
                                Meta
                              </Button>
                              <Button
                                ...
                                disabled={
                                  ['gestor'].includes(user.tipo_usuario) ||
                                  String(user.id) === String(currentUser.id)
                                }
                                title={
                                  ['gestor'].includes(user.tipo_usuario)
                                    ? 'Não é permitido excluir gestor'
                                    : String(user.id) === String(currentUser.id)
          ...
          {metaDialogUser && (
            <div className="space-y-3">
              <div className="text-sm">
                <div>
                  <strong>Nome:</strong> {metaDialogUser.nome}
                </div>
                <div>
                  <strong>Email:</strong> {metaDialogUser.email}
                </div>
                <div>
                  <strong>Tipo:</strong> {metaDialogUser.tipo_usuario}
                </div>
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setMetaForm((prev) => ({
                      ...prev,
                      valor_meta_input: formatMoneyBRInput(e.target.value),
                    }))
                  }
                  placeholder="0,00"
                />
              ...
              <div className="flex justify-end gap-2 pt-2">
                <Button variant="outline" onClick={closeMetaDialog}>
                  Cancelar
                </Button>
                <Button
                  onClick={async () => {
