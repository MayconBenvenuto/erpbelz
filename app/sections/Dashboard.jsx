...
export default function DashboardSection({ currentUser: _currentUser, proposals = [], users = [], userGoals: _userGoals = [], solicitacoes = [] }) {
	...
	useEffect(() => {
		try {
			...
			if (saved) {
				...
				setStatusSolicitacoes(Array.isArray(saved.statusSolicitacoes) ? saved.statusSolicitacoes : [])
	...
	useEffect(() => {
		const state = { dateRange, customFrom, customTo, statusPropostas, statusSolicitacoes, analistaId, operadorasSel, consultorSel }
		try { localStorage.setItem(storageKey, JSON.stringify(state)) } catch {}
	}, [dateRange, customFrom, customTo, statusPropostas, statusSolicitacoes, analistaId, operadorasSel, consultorSel, storageKey])
	...
	const filteredProposals = useMemo(() => {
		...
		const hasStatus = (s) => statusPropostas.length === 0 || statusPropostas.includes(String(s || '').toLowerCase())
		const hasOperadora = (op) => operadorasSel.length === 0 || operadorasSel.includes(String(op || '').toLowerCase())
		const hasAnalista = (id) => !analistaId || String(id || '') === String(analistaId)
		const hasConsultor = (c, e) => !consultorSel || [String(c||''), String(e||'')].map(x=>x.toLowerCase()).includes(String(consultorSel).toLowerCase())
		return proposals.filter(p => within(p.criado_em) && hasStatus(p.status) && hasOperadora(p.operadora) && hasAnalista(p.atendido_por) && hasConsultor(p.consultor, p.consultor_email))
	...
	const filteredSolicitacoes = useMemo(() => {
		...
		const hasStatus = (s) => statusSolicitacoes.length === 0 || statusSolicitacoes.includes(String(s || '').toLowerCase())
		const hasOperadora = (op) => operadorasSel.length === 0 || operadorasSel.includes(String(op || '').toLowerCase())
		const hasAnalista = (id) => !analistaId || String(id || '') === String(analistaId)
		return solicitacoes.filter(s => within(s.criado_em) && hasStatus(s.status) && hasOperadora(s.operadora) && hasAnalista(s.atendido_por))
	...
	// Propostas - métricas macro (usando filtradas)
		...
		const valorImplantado = filteredProposals.filter(p => p.status === 'implantado').reduce((s, p) => s + Number(p.valor || 0), 0)
		const conversao = total > 0 ? Math.round((implantadas / total) * 100) : 0

		// Contagem por status
		const statusCounts = STATUS_OPTIONS.map((st) => {
			const count = filteredProposals.filter(p => (String(p.status || '').toLowerCase() === String(st).toLowerCase())).length
	...
	for (const p of filteredProposals) {
		...
		const series = days.map(({ label, key }) => {
			const criadas = filteredProposals.filter(p => (p.criado_em && new Date(p.criado_em).toISOString().slice(0, 10) === key)).length
			const assumidas = filteredProposals.filter(p => (p.atendido_em && new Date(p.atendido_em).toISOString().slice(0, 10) === key)).length
	...
	for (const p of filteredProposals) {
		...
		const rankingAnalistas = users
			.map(u => ({ id: u.id, nome: u.nome, count: porAnalista.get(String(u.id)) || 0 }))
			.filter(x => x.count > 0)
			.sort((a, b) => b.count - a.count)
			.slice(0, 5)

		return { total, implantadas, valorTotal, valorImplantado, conversao, statusCounts, topOperadoras, series, buckets, rankingAnalistas }
	...
	// Movimentação (solicitações)
		...
		const porStatus = SOLICITACAO_STATUS.map(st => ({ status: st, count: filteredSolicitacoes.filter(s => s.status === st).length }))
		const abertas = filteredSolicitacoes.filter(s => !['concluída', 'cancelada'].includes(s.status))
	...
	// Equipe
		const mEquipe = useMemo(() => {
			const now = Date.now()
			const isOnline = (u) => Boolean(u?.isOnline || (u?.ultimo_refresh && (now - safeDate(u.ultimo_refresh) < 5 * 60 * 1000)))
		...
		const workload = users.map(u => ({
			...
			nome: u.nome,
			online: isOnline(u),
			emAndamento: filteredProposals.filter(p => p.atendido_por && String(p.atendido_por) === String(u.id) && p.status !== 'implantado').length,
			implantadas30d: filteredProposals.filter(p => p.status === 'implantado' && String(p.atendido_por) === String(u.id) && (now - safeDate(p.updated_at || p.atendido_em || p.criado_em)) <= 30 * 24 * 60 * 60 * 1000).length
		}))
		const topWorkload = workload.filter(w => w.emAndamento > 0).sort((a, b) => b.emAndamento - a.emAndamento).slice(0, 5)
		const topImplantadas = workload.filter(w => w.implantadas30d > 0).sort((a, b) => b.implantadas30d - a.implantadas30d).slice(0, 5)
	...
	// Watcher: Propostas sem responsável com SLA estourado (>24h) respeitando filtros
		const propostasAbertasCriticas = useMemo(() => {
			const now = Date.now()
			return filteredProposals.filter(p => !p.atendido_por && p.criado_em && (now - safeDate(p.criado_em) > 24 * 60 * 60 * 1000))
	}, [filteredProposals])
	...
	const cfg = {}
		mPropostas.topOperadoras.forEach(({ operadora }) => {
			...
			cfg[operadora] = {
				label: operadora,
		...(logo ? { icon: () => (<Image src={`/seguradoras/${logo}`} alt={operadora} width={14} height={14} className="h-3 w-auto" />) } : {})
			}
		})
		return cfg
	...
	return (
		<div className="space-y-6 overflow-x-hidden">
			...
			<Card>
					...
					<div className="flex flex-col gap-3">
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
							...
							<div className="flex flex-col gap-1">
								...
								<Input type="date" value={customFrom} onChange={(e)=>setCustomFrom(e.target.value)} disabled={dateRange !== 'custom'} />
							</div>
							<div className="flex flex-col gap-1">
								<span className="text-xs text-muted-foreground">Até</span>
								<Input type="date" value={customTo} onChange={(e)=>setCustomTo(e.target.value)} disabled={dateRange !== 'custom'} />
							</div>
							{/* Analista */}
							<div className="flex flex-col gap-1">
								<span className="text-xs text-muted-foreground">Analista</span>
				<Select value={analistaId || '__all__'} onValueChange={(v)=>setAnalistaId(v === '__all__' ? '' : v)}>
								...
								<DropdownMenu>
									<DropdownMenuTrigger asChild>
										<Button variant="outline" className="justify-between">
											{operadorasSel.length > 0 ? `${operadorasSel.length} selecionada(s)` : 'Todas'}
										</Button>
									</DropdownMenuTrigger>
									<DropdownMenuContent className="w-56">
										...
										{OPERADORAS.map(op => {
											...
											return (
												<DropdownMenuCheckboxItem key={val} checked={checked} onCheckedChange={(v)=>{
													setOperadorasSel(prev => v ? Array.from(new Set([...prev, val])) : prev.filter(x=>x!==val))
												}}>{op}</DropdownMenuCheckboxItem>
											)
										})}
									</DropdownMenuContent>
								</DropdownMenu>
				...
				<Select value={consultorSel || '__all__'} onValueChange={(v)=>setConsultorSel(v === '__all__' ? '' : v)}>
									<SelectTrigger className="w-full"><SelectValue placeholder="Todos" /></SelectTrigger>
									<SelectContent>
					<SelectItem value="__all__">Todos</SelectItem>
										{Array.from(new Set(proposals.map(p => (p.consultor || p.consultor_email || '').toString()).filter(Boolean))).map(c => (
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
							...
							<div className="flex flex-col gap-1">
								...
								<DropdownMenu>
									<DropdownMenuTrigger asChild>
										<Button variant="outline" className="justify-between">
											{statusPropostas.length > 0 ? `${statusPropostas.length} selecionado(s)` : 'Todos'}
										</Button>
									</DropdownMenuTrigger>
									<DropdownMenuContent className="w-56">
										...
										{STATUS_OPTIONS.map(st => {
											...
											return (
												<DropdownMenuCheckboxItem key={val} checked={checked} onCheckedChange={(v)=>{
													setStatusPropostas(prev => v ? Array.from(new Set([...prev, val])) : prev.filter(x=>x!==val))
												}}>{st}</DropdownMenuCheckboxItem>
											)
										})}
									</DropdownMenuContent>
								</DropdownMenu>
							...
							<div className="flex flex-col gap-1">
								...
								<DropdownMenu>
									<DropdownMenuTrigger asChild>
										<Button variant="outline" className="justify-between">
											{statusSolicitacoes.length > 0 ? `${statusSolicitacoes.length} selecionado(s)` : 'Todos'}
										</Button>
									</DropdownMenuTrigger>
									<DropdownMenuContent className="w-56">
										...
										{SOLICITACAO_STATUS.map(st => {
											...
											return (
												<DropdownMenuCheckboxItem key={val} checked={checked} onCheckedChange={(v)=>{
													setStatusSolicitacoes(prev => v ? Array.from(new Set([...prev, val])) : prev.filter(x=>x!==val))
												}}>{st}</DropdownMenuCheckboxItem>
											)
										})}
									</DropdownMenuContent>
							...
							<div className="flex items-end gap-2 lg:col-span-3">
								<Button variant="secondary" onClick={()=>{
									setDateRange('30d'); setCustomFrom(''); setCustomTo(''); setStatusPropostas([]); setStatusSolicitacoes([]); setAnalistaId(''); setOperadorasSel([]); setConsultorSel('')
						...
						<div className="flex flex-wrap gap-2">
							{analistaId && (
								<Badge variant="secondary" className="flex items-center gap-2">Analista: {users.find(u=>String(u.id)===String(analistaId))?.nome || analistaId}
									<button className="ml-1" onClick={()=>setAnalistaId('')}>×</button>
								</Badge>
							)}
							{consultorSel && (
								<Badge variant="secondary" className="flex items-center gap-2">Consultor: {consultorSel}
									<button className="ml-1" onClick={()=>setConsultorSel('')}>×</button>
								</Badge>
							)}
							{operadorasSel.map(op => (
								<Badge key={op} variant="secondary" className="flex items-center gap-2">{op}
									<button className="ml-1" onClick={()=>setOperadorasSel(prev=>prev.filter(x=>x!==op))}>×</button>
								</Badge>
							))}
							{statusPropostas.map(st => (
								<Badge key={`sp-${st}`} variant="secondary" className="flex items-center gap-2">Proposta: {st}
									<button className="ml-1" onClick={()=>setStatusPropostas(prev=>prev.filter(x=>x!==st))}>×</button>
								</Badge>
							))}
							{statusSolicitacoes.map(st => (
								<Badge key={`ss-${st}`} variant="secondary" className="flex items-center gap-2">Solicitação: {st}
									<button className="ml-1" onClick={()=>setStatusSolicitacoes(prev=>prev.filter(x=>x!==st))}>×</button>
								</Badge>
							))}
						</div>
					</div>
			...
			{propostasAbertasCriticas.length > 0 && (
				<Card className="border-l-4 border-l-red-500">
					<CardHeader>
						<CardTitle className="text-red-600">Atenção: Propostas sem responsável há mais de 24 horas</CardTitle>
						<CardDescription>Existem {propostasAbertasCriticas.length} propostas aguardando triagem.</CardDescription>
					</CardHeader>
					<CardContent>
						<div className="flex flex-wrap gap-2">
							{propostasAbertasCriticas.slice(0, 12).map(p => (
								<Badge key={p.id} variant="destructive" className="text-xs">{p.codigo || String(p.id).slice(0, 8)}</Badge>
							))}
							{propostasAbertasCriticas.length > 12 && (
								<span className="text-xs text-muted-foreground">+{propostasAbertasCriticas.length - 12} mais…</span>
							)}
						</div>
					</CardContent>
				</Card>
			...
			<Tabs value={active} onValueChange={setActive}>
				<TabsList className="flex flex-wrap gap-1">
					<TabsTrigger value="geral"><LayoutGrid className="w-4 h-4 mr-1" /> Geral</TabsTrigger>
					<TabsTrigger value="propostas"><FileText className="w-4 h-4 mr-1" /> Propostas</TabsTrigger>
					<TabsTrigger value="movimentacao"><Activity className="w-4 h-4 mr-1" /> Movimentação</TabsTrigger>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
						...
						<Card className="border-l-4 border-l-green-500">
							...
							<CardContent>
								...
								<p className="text-xs text-muted-foreground">{mPropostas.conversao}% de conversão</p>
							</CardContent>
						</Card>
						<Card className="border-l-4 border-l-indigo-500">
							...
							<CardContent>
								<div className="text-2xl font-bold text-indigo-600">{formatCurrency(mPropostas.valorTotal)}</div>
						...
						<Card className="border-l-4 border-l-emerald-500">
							...
							<CardContent>
								<div className="text-2xl font-bold text-emerald-600">{formatCurrency(mPropostas.valorImplantado)}</div>
								<p className="text-xs text-muted-foreground">{mPropostas.valorTotal > 0 ? Math.round((mPropostas.valorImplantado / mPropostas.valorTotal) * 100) : 0}% do total</p>
							</CardContent>
						</Card>
					</div>
							...
							<CardContent>
								<ChartContainer config={chartConfig} className="h-72">
									<LineChart data={mPropostas.series} margin={{ left: 12, right: 12 }}>
										...
										<Line type="monotone" dataKey="assumidas" stroke="#8B5CF6" strokeWidth={2} dot={false} />
							...
							<CardContent>
								{mPropostas.topOperadoras.length === 0 ? (
									...
									<ChartContainer config={operadoraLegendConfig} className="h-72 aspect-auto">
										<PieChart>
											<Pie data={mPropostas.topOperadoras} dataKey="count" nameKey="operadora" innerRadius={48} outerRadius={72} paddingAngle={3}>
											...
											<Legend content={<ChartLegendContent nameKey="operadora" className="flex flex-wrap gap-3 justify-center" />} />
										</PieChart>
									</ChartContainer>
								)}
							</CardContent>
							...
							<CardContent>
								<ChartContainer config={chartConfig} className="h-72">
									<BarChart data={mPropostas.statusCounts} margin={{ left: 12, right: 12 }}>
										<CartesianGrid strokeDasharray="3 3" />
										<XAxis dataKey="status" tickLine={false} axisLine={false} tickFormatter={(v)=>String(v).slice(0,12)} />
										<YAxis allowDecimals={false} width={28} />
										<ChartTooltip content={<ChartTooltipContent />} />
				...
				<TabsContent value="propostas" className="space-y-6">
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
						<Card className="border-l-4 border-l-blue-500">
							...
							<CardContent>
								<div className="text-2xl font-bold text-blue-600">{filteredProposals.filter(p => p.status !== 'implantado').length}</div>
						...
						<Card className="border-l-4 border-l-amber-500">
							...
							<CardContent>
								<div className="text-2xl font-bold text-amber-600">{filteredProposals.filter(p => !p.atendido_por).length}</div>
						...
						<Card className="border-l-4 border-l-purple-500">
							...
							<CardContent>
								{(() => {
									...
									return (
										<>
											<div className="text-2xl font-bold text-purple-600">{formatCurrency(ticket)}</div>
							...
							<CardContent>
								<div className="grid grid-cols-2 gap-2">
									{mPropostas.statusCounts.map((s) => {
										const c = STATUS_COLORS[s.status] || { bg: '#f6f6f6', text: '#333', border: '#e2e2e2' }
										return (
											<div key={s.status} className="p-3 rounded-md border text-xs" style={{ backgroundColor: c.bg, color: c.text, borderColor: c.border }}>
												<div className="flex items-center justify-between">
			...
			<Card className="border-l-4 border-l-indigo-500">
							...
							<CardContent>
				<div className="text-2xl font-bold text-indigo-600">{filteredSolicitacoes.filter(s => s.status === 'concluída').length}</div>
							...
							<CardContent>
								<ul className="space-y-2 text-sm">
									{mMov.porStatus.map(s => (
										<li key={s.status} className="flex items-center justify-between p-2 rounded-md border">
											<span className="capitalize">{s.status}</span>
				...
				<TabsContent value="equipe" className="space-y-6">
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
						...
						<Card className="border-l-4 border-l-blue-500">
							...
							<CardContent>
								<div className="text-2xl font-bold text-blue-600">{mEquipe.topWorkload[0]?.nome || '—'}</div>
								<p className="text-xs text-muted-foreground">{mEquipe.topWorkload[0]?.emAndamento || 0} propostas</p>
							</CardContent>
						</Card>
						<Card className="border-l-4 border-l-indigo-500">
							<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
							...
							<CardContent>
								<div className="text-2xl font-bold text-indigo-600">{mEquipe.topImplantadas.reduce((a,b)=>a+b.implantadas30d,0)}</div>
								...
								{mEquipe.topWorkload.length === 0 ? (
									...
									<ul className="space-y-2">
										{mEquipe.topWorkload.map(w => (
											<li key={w.id} className="flex items-center justify-between p-2 rounded-md border">
												<div className="flex items-center gap-2">
													<span className={`w-2 h-2 rounded-full ${w.online ? 'bg-emerald-500' : 'bg-zinc-400'}`} />
