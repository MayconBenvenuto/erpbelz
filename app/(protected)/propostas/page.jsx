...
export default function PropostasPage() {
  ...
  const handleUpdateProposalStatus = async (id, status, _proposal) => {
    ...
    if (previousData && Array.isArray(previousData)) {
      const optimisticData = previousData.map((p) => (p.id === id ? { ...p, status } : p))
  ...
  const handlePatchProposal = async (id, data) => {
    ...
    if (previousData && Array.isArray(previousData)) {
      const optimisticData = previousData.map((p) => (p.id === id ? { ...p, ...data } : p))
      queryClient.setQueryData(queryKeys.proposalsFiltered({}), optimisticData)
    }
    ...
    try {
      ...
      if (previousData && Array.isArray(previousData)) {
        const updatedData = previousData.map((p) => (p.id === id ? result : p))
