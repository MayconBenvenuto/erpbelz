...
const DEFAULT_LIST_COLUMNS =
  'id,codigo,cnpj,cliente_nome,cliente_email,cliente_telefone,consultor,consultor_email,operadora,quantidade_vidas,valor,status,criado_por,atendido_por,criado_em,updated_at,atendido_em,observacoes'
    ...
    if (hasExplicitPagination) {
      return cacheJson(
        ...
        origin,
        {
          ...
          took_ms: durationMs ?? undefined,
        },
        { maxAge: 60, swr: 300, headers: durationHeader }
      )
    }
  } catch {}
...
export async function POST(request) {
  ...
  if (!hasPermission(auth.user, 'createPropostas')) {
    return handleCORS(
      NextResponse.json({ error: 'Acesso negado para criar propostas' }, { status: 403 }),
      origin
    )
  }
  ...
  if (auth.user.tipo_usuario === 'consultor') {
    ...
    if (!payload.consultor_email) {
      return handleCORS(
        NextResponse.json({ error: 'Email do consultor ausente' }, { status: 400 }),
        origin
      )
    }
    ...
    if (!payload.cliente_nome || !payload.cliente_email) {
      return handleCORS(
        NextResponse.json({ error: 'Informe nome e email do cliente' }, { status: 400 }),
        origin
      )
    }
    ...
    if (!payload.consultor || !payload.consultor_email) {
      return handleCORS(
        NextResponse.json(
          { error: 'Consultor e email do consultor são obrigatórios' },
          { status: 400 }
        ),
        origin
      )
