...
export async function POST(request) {
  ...
  if (!body?.proposta_id || !body?.tag)
    return handleCORS(NextResponse.json({ error: 'Dados incompletos' }, { status: 400 }), origin)
  ...
  const { error } = await supabase
    .from('propostas_tags')
    .insert({ proposta_id: body.proposta_id, tag })
...
export async function DELETE(request) {
  ...
  if (!proposta_id || !tag)
    return handleCORS(NextResponse.json({ error: 'Parâmetros faltando' }, { status: 400 }), origin)
  const { error } = await supabase
    .from('propostas_tags')
    .delete()
    .eq('proposta_id', proposta_id)
