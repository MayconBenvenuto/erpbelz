...
import { cacheJson, supabase } from '@/lib/api-helpers'
  ...
  try {
    ...
    const columns =
      fields === 'list'
        ...
        : '*'
    const onlyOverdue = ['1', 'true', 'yes'].includes(
      String(searchParams.get('atrasadas') || '').toLowerCase()
    )
    ...
    if (userIds.length) {
      try {
        ...
        data.forEach((d) => {
          ...
          if (u) {
            d.criado_por_nome = u.nome
            d.criado_por_email = u.email
          }
        })
      } catch (_) {}
    }
...
export async function POST(req) {
  try {
    ...
    if (
      body.tipo === 'inclusao' &&
      !['funcionario', 'socio', 'dependente'].includes(body.subtipo || '')
    ) {
    ...
    if (body.sla_previsto) {
      ...
      if (isNaN(d.getTime()))
        return NextResponse.json({ message: 'sla_previsto inválido' }, { status: 400 })
      slaPrevisto = d.toISOString().slice(0, 10)
    }
    // Validação básica dos arquivos (se existirem)
    const safePath = (p) =>
      typeof p === 'string' && !p.startsWith('/') && !p.includes('..') && p.length < 200
    const arquivosSan = (Array.isArray(body.arquivos) ? body.arquivos : []).filter(
      (a) => a && safePath(a.path || '')
    )
      ...
      if (/column \"status\"|column \"sla_previsto\"|column \"historico\"/.test(msg)) {
        ...
        if (legacyErr) {
          return NextResponse.json(
            { message: 'Erro ao criar (migração ausente?)', detalhe: legacyErr.message },
            { status: 500 }
          )
        }
        return NextResponse.json(
          {
            message:
              'Solicitação criada (modo legado - aplique migração 20250822_add_solicitacoes_table.sql)',
            data: legacyData,
            aviso: 'Migração solicitacoes não aplicada',
          },
          { status: 201 }
        )
      }
    ...
    console.error(
      ...
      sanitizeForLog({ message: e?.message, stack: e?.stack })
    )
